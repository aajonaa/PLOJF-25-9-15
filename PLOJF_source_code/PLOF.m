function [best_pos, Convergence_curve] = XXPLO_variant1_EPM(N, MaxFEs, lb, ub, dim, fobj)
% XXPLO_variant1_EPM: PLO-based variant with EPM Strategy 2 (adaptive, conditional FDB) ONLY
% - Baseline generator preserved: PLO-style LS/GS two-candidate with partner-guided direction H
% - EPM-inspired addition: Adaptive, FE-driven, conditional FDB survivor selection on [parents; offspring]
% - No JADE archive: removes archive A injection and archive updates entirely
% - Goal: isolate the contribution of EPM Strategy 2 against the PLO baseline
...
% Adaptive FDB trigger (function of FEs)
...
while FEs < MaxFEs
    ...
    for i = 1:N
        ...
        % Accept-if-better per individual (no archive updates here)
        if f_cand < Fitness(i)
