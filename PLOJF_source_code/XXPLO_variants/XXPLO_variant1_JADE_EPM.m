function [best_pos, Convergence_curve] = XXPLO_variant1_JADE_EPM(N, MaxFEs, lb, ub, dim, fobj)
% XXPLO_variant1_JADE_EPM: JADE-archive PLO variant + EPM Strategy 2 (conditional FDB fallback)
% - Baseline: XXPLO_variant1_JADE (keep PLO-style generator and archive injection UNTOUCHED)
% - Added (EPM-inspired): When stagnated and diversity is low, apply a one-shot FDB survivor selection
%   on the doubled population [parents; offspring] without extra evaluations.
% - Goal: improve robustness and late-stage convergence while preserving the baseline's behavior.
...
% PLO parameters and adaptation (same as baseline variant1_JADE)
phi_qKR = 0.25 + 0.55 * ((0 + ((1:N)/N)) .^ 0.5); %#ok<NASGU>
succ_win = 30; succ_hist = false(1, succ_win); succ_ptr = 1;
target_succ = 0.25; adapt_gain = 0.30; adapt_scale = 1.0;

% Partnered direction weights (same)
c_mean=0.50; c_pbest=0.80; c_gbest=0.80; c_partner=0.60; early_ratio=0.20;

% Anti-stagnation controls (same)
no_improv_cnt=0; no_improv_limit=8; imm_frac=0.15; div_thresh=0.01;

% Adaptive FDB trigger (as a function of FEs)
base_no_improv = 12;   % early conservative, later more aggressive
alpha_lim      = 0.7;  % shrink factor of stall limit over progress
base_div_thr   = div_thresh; % base diversity threshold
beta_div       = 0.5;  % diversity threshold growth over progress
fdb_on_t       = 0.40; % enable FDB after 40% of FEs
pmax_fdb       = 0.6;  % cap for trigger probability
cooldown_iters = 3;    % cooldown after a FDB application
cooldown       = 0;

% JADE/SHADE-style archive (same as baseline variant)
...
while FEs < MaxFEs
    ...
    for i = 1:N
        % PLO generator (unchanged): LS + GS + partner-guided H, two-candidate
        ...
        % two candidates (cand1 exploit, cand2 explore with archive injection)
        ori = rand(1,dim);
        ...
        % Accept-if-better per individual (unchanged); push replaced parent to archive
        if f_cand < Fitness(i)
    ...
    % Success history (same baseline semantics)
    succ_hist(succ_ptr) = any(newF < Fitness); succ_ptr = succ_ptr + 1; if succ_ptr > succ_win, succ_ptr = 1; end

    % EPM Strategy 2 (added): Conditional FDB survivor selection on [X; newX]
    % Trigger when stagnated and diversity collapsed; no extra evaluations are needed.
    ...
    % Adaptive FDB trigger based on fraction of FEs
    t = FEs / max(1, MaxFEs);
    if t >= fdb_on_t && cooldown == 0
        % adapt stall limit and diversity threshold with progress
        stall_lim = round(base_no_improv * max(0.3, 1 - alpha_lim * t));
        div_thr   = base_div_thr * (1 + beta_div * t);
        % probabilistic trigger when near/over limits
        ...
        if (no_improv_cnt >= stall_lim && low_div) || (near_stall && low_div && rand < p_trig)
            ...
            newX = X_sel; newF = F_sel;   % override finalize targets
            no_improv_cnt = 0;            % reset after fallback
            cooldown = cooldown_iters;     % start cooldown to avoid overuse
        end
    end
    if cooldown > 0, cooldown = cooldown - 1; end

    % finalize generation (unchanged)
    X = newX; Fitness = newF;
