function [best_pos, Convergence_curve] = XXPLO_PB_RTR_k(N, MaxFEs, lb, ub, dim, fobj)
% XXPLO_PB_RTR_k: XXPLO_PB_RTR + adaptive kRTR only (no archive, no acc-rate, no FDB fallback)
% - Proposal: current-to-pbest/1 (no archive)
% - Replacement: RTR with mild diversity-aware kRTR
% - Success history: boolean global-improvement
% - kRTR: adaptive via norm_div
...
while FEs < MaxFEs
    ...
    % RTR replacement with adaptive kRTR
    improved_global = false;
    idx_order = randperm(N);
    kRTR_now = max(3, min(12, round(min(5 + 20*max(0, 0.02 - norm_div), N))));
    for t = 1:N
        i = idx_order(t);
        cand_idx = randperm(N, kRTR_now);
