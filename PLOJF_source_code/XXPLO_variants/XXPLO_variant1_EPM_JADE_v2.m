function [best_pos, Convergence_curve] = XXPLO_variant1_EPM_JADE_v2(N, MaxFEs, lb, ub, dim, fobj)
% Polar Lights Optimization with Event-Driven JADE Integration and Fitness-Distance Balanced Selection
% Enhanced Polar Lights Optimization via Event-Driven Differential Evolution and Adaptive Population Management
% event-driven JADE integration and adaptive population management through FDB (Fitness-Distance Balanced)
% XXPLO_variant1_EPM_JADE_v2: Event-driven, orthogonal, bottom-only JADE injection on EPM baseline
% - Baseline: XXPLO_variant1_EPM (PLO LS/GS generator + adaptive conditional FDB)
% - JADE usage policy (refined for synergy):
%   * Only for K generations right AFTER an FDB trigger (event-driven window)
%   * Only applied to the bottom-q fraction individuals (protect elites)
%   * Orthogonalize injection to H (hybrid direction) to avoid conflicting exploitation
%   * Small probability and magnitude (conservative)
% - Goal: improve over XXPLO_variant1_EPM
