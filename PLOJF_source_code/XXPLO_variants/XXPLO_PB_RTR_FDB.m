function [best_pos, Convergence_curve] = XXPLO_PB_RTR_FDB(N, MaxFEs, lb, ub, dim, fobj)
% XXPLO_PB_RTR_FDB: XXPLO_PB_RTR + low-frequency FDB fallback (no archive, no acc-rate, fixed kRTR)
% - Proposal: current-to-pbest/1 (no archive)
% - Replacement: RTR; and when stagnated with low diversity, a one-shot FDB survivor selection
...
while FEs < MaxFEs
    ...
    % stagnation tracking
    if improved_global, no_improv_cnt = 0; else, no_improv_cnt = no_improv_cnt + 1; end

    % One-shot FDB fallback when stagnated + low diversity
    ...
    % success-history update (boolean)
