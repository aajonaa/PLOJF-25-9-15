function [best_pos, Convergence_curve] = XXPLO_PB_RTR_ACC(N, MaxFEs, lb, ub, dim, fobj)
% XXPLO_PB_RTR_ACC: XXPLO_PB_RTR + acceptance-rate success history (no archive, no adaptive kRTR, no FDB fallback)
% - Proposal: current-to-pbest/1 (no archive)
% - Replacement: Restricted Tournament Replacement (RTR)
% - Success history: acceptance rate (replace_count/N)
% - kRTR: fixed
...
while FEs < MaxFEs
    ...
    % RTR replacement
    improved_global = false; replace_count = 0;
    ...
    for t = 1:N
        ...
        j = cand_idx(mpos);
        if newF(i) < Fitness(j)
            X(j,:) = newX(i,:); Fitness(j) = newF(i); age(j) = 0; replace_count = replace_count + 1;
    ...
    % success-history update (acceptance rate)
    acc_rate = replace_count / N;
    succ_hist(succ_ptr) = acc_rate; succ_ptr = succ_ptr + 1; if succ_ptr > succ_win, succ_ptr = 1; end
